...
//============ SHIRLEY: modified veresion that has smaller value sizes so they can be inlined ============
...
static constexpr int kTotal = 10;
static constexpr int kNrMSBContentionKey = 6; // shirley note: default = 6.
...
{
  auto dbv = vhandle.Read<Ycsb::Value>();
  dbv.v.assign(Client::zero_data, 72); // shirley: change to 72 to inline
  dbv.v.resize_junk(72); // shirley: change to 72 to inline
  // dbv.v.assign(Client::zero_data, 100);
  // dbv.v.resize_junk(999);
  vhandle.Write(dbv);
}
        ...
        [this](auto part, auto root, const auto &t) {
          root->AttachRoutine(
              ...
              [](auto &ctx) {
                ...
                // shirley: change to 72 to inline
          ...
          if (i == kTotal - Client::g_extra_read) {
            ...
            root->AttachRoutine(
                ...
                [](auto &ctx) {
                  ...
                  v.v.resize_junk(72); // shirley: change to 72 to inline
                  // v.v.resize_junk(90);
                  vhandle.Write(v);
                  state->signal.fetch_add(1);
                }, part);
