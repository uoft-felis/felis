...
static constexpr int kNrMSBContentionKey = 7;
...
#ifdef YCSB_SMALL_VALUES
  dbv.v.assign(Client::zero_data, 64);
  dbv.v.resize_junk(64);
#else
...
#ifdef YCSB_SMALL_VALUES
                static thread_local volatile char buffer[64];
                std::copy(dbv.v.data(), dbv.v.data() + 64, buffer);
#else
...
#ifdef YCSB_SMALL_VALUES
                  std::copy(v.v.data(), v.v.data() + 64, buffer);
#else
