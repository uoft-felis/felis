...
public:
                        ...
                        int nworkers = 1)
    {
        ...
        std::for_each(flush_worker_futures.begin(), flush_worker_futures.end(),
                      [](std::future<void> &f) { f.get(); });
        // shirley: why does flush time include radix tree rebuild time? but it's okay bc rebuild is short.
...
struct log_record
{
    ...
    key_type key;
    value_type val;
    // size_t h; // shirley: removed. is this unnecessary???
    op_type type;
    char cacheline_padding[cacheline_size - sizeof(type) - sizeof(key) -
                           sizeof(val) /*- sizeof(h)*/]; // shirley: removed h.
...
struct log_record_pm
{
    ...
    // char cacheline_padding[cacheline_size - sizeof(type) - sizeof(key) -
    //                        sizeof(val) - sizeof(h)];
    log_record_pm(op_type type, const key_type &key, const value_type &val)
        : type(type), key(key), val(val) {}

    log_record_pm(op_type type, const key_type &key) : type(type), key(key), val() {}
};
...
class durable_concur_buffer_btree
{
  public:
    ...
    durable_concur_buffer_btree(size_t expected_size, caching_log_page_allocator<> *allocator) : approx_size(0), actual_size(0)
    {
        ...
        pmlogs.resize(pmem_log_worker_num);
        for (int i = 0; i < pmem_log_worker_num; i++) {
           nvm_dram_alloc(&(pmlogs[i]), 256, 64*1024*1024);
        }
    }
    ...
    // shirley: this function constructs "pmem_log_worker_num" # of pm logs using dram logs with "pmem_log_worker_num" threads
    void dramlog_to_pmlog() {
        // threads fetch add entry_cnt to check which log stripe they can do next.
        ...
        auto log_worker_func = [&, this](int thread_id) {
            ...
            while (true) {
                ...
                log_type *dramlog = logs[lognum];
                
                // shirley: this is modified from the function for_each_record in class lockfree_pwal.
                std::vector<log_page<64 * 1024>*> pages;
                auto p = dramlog->tailp;
                while (p) {
                    pages.push_back(p);
                    p = p->next;
                }
                for (int i = (int)pages.size() - 1; i >= 0; --i) {
                    ...
                    while (off + sizeof(log_record<key_type, value_type>) <= log_page<64 * 1024>::effective_storage_size) {
                        ...
                        *my_log = log_record_pm<key_type, value_type>(
                        ...
                        off += sizeof(log_record<key_type, value_type>);
                    }
                }
            }
  ...
  private:
    ...
    std::atomic_int actual_size = 0;
    // persistent states
    std::vector<log_type *> logs;
    size_t cap;
    std::vector<void *> pmlogs;
};

// shirley: this class (durable_concur_buffer_art) is not being used. btree is used for buffer tree.
// template <class Key, class Value>
...
class concur_dptree
{
  public:
    ...
    // inline entry_pair make_upsert_value(entry_pair v) {
    ...
    void upsert(const key_type & key, const value_type & value) {
        ...
        buffer_btree_type *buffer_tree;
        {
            ...
            buffer_tree->update(key, (value), should_insert_f_cpbt);
        }
    ...
    void construct_pmlog(bool use_dram_log = false) {
        if (use_dram_log) {
            ...
            front_buffer_tree->entries_to_pmlog();
        }
    }
